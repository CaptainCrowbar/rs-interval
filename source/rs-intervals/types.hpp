...
#define RS_INTERVALS_ENUM_CLASS(EnumType, IntType, first_value, first_name, ...) \
...
namespace RS::Intervals {
    ...
    namespace Detail {
        ...
        template <typename T>
        concept OstreamFormattable = requires (const T& t, std::ostream& o) {
            { o << t };
        };
    ...
    template <typename T>
    concept IntervalCompatible =
        Continuous<T> || Integral<T> || Ordered<T> || Stepwise<T>;
    ...
    struct DefaultFormatter {
        ...
        template <Detail::OstreamFormattable T>
        requires (! std::integral<T> && ! std::convertible_to<T, std::string>)
        std::string operator()(const T& t) const {
    ...
    namespace Detail {
        ...
        RS_INTERVALS_ENUM_CLASS(BoundaryType, int, -3,
            empty,
            minus_infinity,
            closed,
            open,
        ...
        struct Boundary {
            ...
            std::string Boundary<T>::str(const F& f) const {
                using BT = BoundaryType;
                switch (type) {
