...
void test_rs_interval_integral_interval_arithmetic() {
    ...
    TRY(in = - Itv(1,5,"()"));  TRY(str = std::format("{}", in));  TEST_EQUAL(str, "[-4,-2]");
    TRY(in = - Itv(1,5,"[)"));  TRY(str = std::format("{}", in));  TEST_EQUAL(str, "[-4,-1]");
    TRY(in = - Itv(1,5,"(]"));  TRY(str = std::format("{}", in));  TEST_EQUAL(str, "[-5,-2]");
    TRY(in = - Itv(0,5,"<"));   TRY(str = std::format("{}", in));  TEST_EQUAL(str, ">=-4");
    TRY(in = - Itv(0,5,"<="));  TRY(str = std::format("{}", in));  TEST_EQUAL(str, ">=-5");
    TRY(in = - Itv(5,0,">"));   TRY(str = std::format("{}", in));  TEST_EQUAL(str, "<=-6");
    TRY(in = - Itv(5,0,">="));  TRY(str = std::format("{}", in));  TEST_EQUAL(str, "<=-5");

    TRY(in = Itv() + 42);           TRY(str = std::format("{}", in));  TEST_EQUAL(str, "{}");
    TRY(in = Itv::all() + 42);      TRY(str = std::format("{}", in));  TEST_EQUAL(str, "*");
    TRY(in = Itv(5) + 42);          TRY(str = std::format("{}", in));  TEST_EQUAL(str, "47");
    TRY(in = Itv(5,10,"[]") + 42);  TRY(str = std::format("{}", in));  TEST_EQUAL(str, "[47,52]");
    TRY(in = Itv(5,10,"()") + 42);  TRY(str = std::format("{}", in));  TEST_EQUAL(str, "[48,51]");
    TRY(in = Itv(5,10,"[)") + 42);  TRY(str = std::format("{}", in));  TEST_EQUAL(str, "[47,51]");
    TRY(in = Itv(5,10,"(]") + 42);  TRY(str = std::format("{}", in));  TEST_EQUAL(str, "[48,52]");
    TRY(in = Itv(5,10,"<") + 42);   TRY(str = std::format("{}", in));  TEST_EQUAL(str, "<=51");
    TRY(in = Itv(5,10,"<=") + 42);  TRY(str = std::format("{}", in));  TEST_EQUAL(str, "<=52");
    TRY(in = Itv(5,10,">") + 42);   TRY(str = std::format("{}", in));  TEST_EQUAL(str, ">=48");
    TRY(in = Itv(5,10,">=") + 42);  TRY(str = std::format("{}", in));  TEST_EQUAL(str, ">=47");
